...
class ShowScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
            ...
            title: const Text("Fix MicroWave",  style: TextStyle(fontWeight: FontWeight.bold, color: black)),
            centerTitle: true,
        ),
  ...
  Row _reviewContainer() {
    return Row(
              children: [
                Expanded(
                    child: _button(const Text("Reviews", style: TextStyle(fontSize: fontSizeX2, fontWeight: FontWeight.bold, color: Colors.brown)), color: lightRed)),
                const SizedBox(width: kPadding*3),
                _button(const Icon(Icons.message_rounded, color: Colors.white, size: 25),
                    color: black)
              ],
            );
  }
            ...
            padding:const EdgeInsets.all(kPadding*1.45),
             decoration: const BoxDecoration(
              ...
              child: Row(children: <Widget>[
                Text(lText, style: const TextStyle(fontSize: fontSizeX1, fontWeight: FontWeight.bold)),
                const Spacer(),
                Text(rText, style: const TextStyle(fontSize: fontSizeX1, fontWeight: FontWeight.bold))
            ],));
  }
  ...
  Container _displayContainer(Size _size) {
    return Container(
            ...
            child: Container(
              decoration: const BoxDecoration(
                boxShadow: <BoxShadow>[BoxShadow(color: Colors.black38, offset:Offset(0,15) ,blurRadius:20, spreadRadius: 10)]
              ),
              child: const Image(
                  image: AssetImage("assets/images/microwave.png")),
            ),
  ...
  Widget _button(Widget child, {required Color color}) {
    return GestureDetector(
          ...
          child: Container(
        alignment: Alignment.center,
        padding: const EdgeInsets.symmetric(vertical:kPadding*1.3, horizontal: kPadding*1.5),
        decoration: BoxDecoration(
            color: color,
            borderRadius: const BorderRadius.all(Radius.circular(10)),
            boxShadow: const <BoxShadow>[
              BoxShadow(offset: Offset(0, 0), blurRadius: 2, color: Colors.black26)
            ]),
        child: child,
      ),
    );
